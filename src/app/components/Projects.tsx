...
// Helper to fix image paths. In Next.js, images in `public` are served at root.
const fixImagePath = (src) => {
    if (src.startsWith('process.env.PUBLIC_URL')) {
        // This is unlikely as I'm replacing it in the array below.
        // But safe to have logic if passed from elsewhere.
        return src.replace('process.env.PUBLIC_URL', '');
    }
    return src;
};
                            ...
                            <div className="flex mx-auto flex-wrap gap-2 overflow-x-auto">
                                {uniqueTechStacks.map((tech) => (
                                    <button
                                        ...
                                        className={`px-4 py-2 rounded-full transition-all duration-300 ${activeFilter === tech
